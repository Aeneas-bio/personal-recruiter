...
1. **A folder or set of files of their CVs/resumes** — including old versions if they have them; a resume that changed over 3 jobs tells you more about trajectory than the latest one alone. Ask where these actually live and get a concrete path, not a vague "I have some somewhere": a **local folder or file path** Claude can read directly, or a **cloud location** (Google Drive, Box, etc.). If it's cloud, confirm the matching connector is available (from Step 1's capability check) before asking for the path or folder ID — if no matching connector is connected, tell the person plainly and ask them to point to a local copy instead rather than guessing at access. Read every file found, not just the newest one — the point is the trajectory across versions, not the latest snapshot.
2. **A folder or set of cover letters**, if they have any saved — same local-path-or-cloud-location question as above, including the connector check for a cloud source. Cover letters are the best source for a person's actual voice and stated motivation — mine them for language, not just facts.
...
Ask for their history of past applications. A URL is the best case, but it is not a requirement — **title + company + a rough date is a complete, first-class entry on its own**; don't let a missing link stop someone from including a role, and don't frame the question as "send me URLs" in a way that implies roles without one don't count. Explicitly say it's fine, even useful, to include ones they were rejected from or that never called back. Rejections are some of the highest-signal data available: they tell you what looked right on paper but wasn't. For each entry:
1. If a URL exists, open it. If it doesn't (dead link, or the person never had one — a referral, a role sourced by a recruiter, or simply not saved), search for the title + company instead. If neither the link nor a search recovers the original posting, proceed on title + company + the person's own description of the role — extract what you can this way, and ask the person directly for anything the posting would have told you (responsibilities, stated requirements) that you can't recover otherwise. A recovered posting or a recalled one both count; don't drop an entry just because it isn't backed by a live link.
